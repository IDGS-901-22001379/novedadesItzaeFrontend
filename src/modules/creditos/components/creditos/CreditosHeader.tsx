// src/modules/creditos/components/creditos/CreditosHeader.tsx
// Encabezado de la pantalla Créditos.
// Responsabilidades: título, resumen centrado y botón principal (nuevo crédito).
// Nota: el resumen queda centrado en medio, y el botón usa verde #34f334.

import type { CreditosTheme } from "../../theme/creditosTheme";

type Props = {
  theme: CreditosTheme;
  resumen: {
    total: number;
    pendientes: number;
    parciales: number;
    pagados: number;
    vencidos: number;
  };
  loading: boolean;
  onNuevo: () => void;
};

export default function CreditosHeader({ resumen, loading, onNuevo }: Props) {
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:items-center">
      {/* Izquierda: título más grande */}
      <div className="sm:justify-self-start">
        <h1 className="text-2xl font-extrabold tracking-tight">Créditos</h1>
      </div>

      {/* Centro: resumen centrado y mismo nivel visual */}
      <div className="sm:justify-self-center">
        <div className="text-center text-lg font-extrabold tracking-tight opacity-95">
          <span>
            Total: <span className="font-extrabold">{resumen.total}</span>
          </span>
          <span className="mx-2">·</span>
          <span>
            Pendientes:{" "}
            <span className="font-extrabold">{resumen.pendientes}</span>
          </span>
          <span className="mx-2">·</span>
          <span>
            Parciales:{" "}
            <span className="font-extrabold">{resumen.parciales}</span>
          </span>
          <span className="mx-2">·</span>
          <span>
            Pagados: <span className="font-extrabold">{resumen.pagados}</span>
          </span>
          <span className="mx-2">·</span>
          <span>
            Vencidos: <span className="font-extrabold">{resumen.vencidos}</span>
          </span>
        </div>
      </div>

      {/* Derecha: botón verde del ejemplo */}
      <div className="sm:justify-self-end">
        <button
          type="button"
          onClick={onNuevo}
          disabled={loading}
          className={[
            "rounded-xl px-4 py-2 text-sm font-extrabold shadow-sm transition disabled:opacity-50",
            "bg-[#34f334] text-[#0b2b0b] hover:bg-[#2fe72f]",
          ].join(" ")}
        >
          Nuevo crédito
        </button>
      </div>
    </div>
  );
}
