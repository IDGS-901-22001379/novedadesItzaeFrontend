// src/modules/empleados/theme/empleadosTheme.ts
// Tema visual del módulo Empleados.
// Responsabilidades: traducir el content_bg del Topbar a clases Tailwind y colores consistentes.
// Nota: se reutilizan EXACTAMENTE los mismos colores del módulo Usuarios (no cambiar paleta).

export type ContentBg = "light" | "dark" | "blue" | "green" | "candy";

export type EmpleadosTheme = {
  titleText: string;
...
const THEMES: Record<ContentBg, EmpleadosTheme> = {
  light: {
  ...
  blue: {
    ...
    // Azul estilo Sidebar (más fuerte, sin verse oscuro)
    headerBg: "bg-[#2F6FED]",
    headerText: "text-white",
  ...
  green: {
    ...
    // Verde estilo Sidebar (más fuerte, sin verse oscuro)
    headerBg: "bg-[#22C55E]",
    headerText: "text-white",
  ...
  candy: {
    ...
    // Rosa/Morado estilo Sidebar (más fuerte, sin verse oscuro)
    headerBg: "bg-[#C026D3]",
    headerText: "text-white",
...
export function getEmpleadosThemeFromContentBg(contentBg: ContentBg): EmpleadosTheme {
